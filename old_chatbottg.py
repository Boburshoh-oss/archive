        ...
        for i in datasss['intents']:
            if i['tag'] == tag:
                if type(i["responses"]) == list:
                    list_random_response = np.random.choice(i["responses"])
                    api = MuxlisaApi(text=list_random_response)
                    response = list_random_response
                else:
...
def chat_text(message):
    ...
    fallback_responses = ["Kechirasiz, savolingizni tushunaolmadim.", "Kechirasiz, Savolingizni qaytara olasizmi?"]
    res = model.predict(keras.preprocessing.sequence.pad_sequences(tokenizer.texts_to_sequences([input_message]),
                                                                   truncating='post', maxlen=max_len))
